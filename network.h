...
void WaitNetwork();
int Send(void*); //an abstract function that would eventually send 256 bytes over the network in a particular order like 0-3 damageDealt, 4-7 posX, 8-11 posY, 12-15 velX, 16-19 velY, etc
int Receive(void*); //this function would receive the info, apply damage via health -= damageDealt (rather than setting the health), set the positions + vel*elapsedTime (since timestamp was received), and set the velocity minus any gravity adjustments
